...
parser.add_argument('--device', type=str, default='cuda:0',
                    help='the text put into torch.device(), use CUDA or cpu')
parser.add_argument('--log_interval', type=int, default=200,
                    help='report interval')
parser.add_argument('--checkpoint', type=str, default='./pepler/',
                    help='directory to save the final model')
parser.add_argument('--outf', type=str, default='generated.jsonl',
...
if torch.cuda.is_available():
    if not args.device.startswith('cuda'):
        print(now_time() + 'WARNING: You have a CUDA device, so you should probably run with --device=cuda:0')
...
# decide whether to train or not by checking checkpoint arg
if not os.path.exists(model_path):
    print(now_time() + 'No checkpoint found | Start training')
    model_path = execute_training_loop()
# Load the best saved model or the checkpoint
...
import pandas as pd
import json
# PEPLER-D does not do the rating prediction subtask.
df = pd.DataFrame({'real': text_test, 'fake': text_predict,
                'predicted_features': feature_batch,
                'context': tokens_context,
                'predicted_rating': [None for _ in range(len(text_test))]})
# df to json
