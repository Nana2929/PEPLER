...
def generate(data):
    ...
    with torch.no_grad():
        while True:
            ...
            #!! make sure what its input `text` is
...
# feature_batch is the set of predicted features (actually just the
# set of features that appear in the predicted text)
...
import pandas as pd
import json
df = pd.DataFrame({'real': text_test, 'fake': text_predict, 'predicted_features': feature_batch,
              'gold_features': test_data.feature, 'gold_rating': test_data.rating.tolist()}
             )
# df to json
